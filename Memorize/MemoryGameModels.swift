...
struct MemoryGame<CardContent> where CardContent: Equatable {  // Generic over any type; we are calling whatever the type is "CardContent"
    ...
    struct Card : Identifiable {
        ...
        // (i.e., not including the current time it's been face up if is currently face up)
        ...
        var isConsuminBonusTime: Bool {
            state() == .faceUpAndUnmatched && bonusTimeRemaining > 0
        }
        ...
        var isConsumingBonusTime = false
